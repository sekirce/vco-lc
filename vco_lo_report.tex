...
\subsection{Definition of VCO}

A VCO, a voltage-controlled signal generator, is a sine-like signal source consisting of two switchable LC circuits with varicaps, frequency tuning banks, and an output buffer. The VCO must have a shutdown mode. The VCO parameters are shown in Table X. Figure Y shows a block diagram of the VCO.

% Should drow a block diagram for VCO.
...
\begin{itemize}
	\item Selective feedback (L, C, Transformer)
	\item Colpitts (2C + 1L)
	\item Clapp (2C+1LC)
	\item Armstrong (1C + xfmr)
	\item Hartley (1C + 2L)
	\item Negative gm or cross-coupled with tuned amplifier and unity feedback.
\end{itemize}


\subsection{Introduction for Hartley VCO LC}

Among the building blocks of a radio frequency (RF) transceiver, the voltage controlled oscillator (VCO) is an important block since it affects the system performances of the transceiver to a large extent. In the past, many approaches have been taken to design integrated VCOs. Cross-coupled oscillators have been preferred over other topologies due to their ease of implementation and differential operation. The Colpitts oscillator [1] can potentially achieve lower phase noise due to superior cyclostationary noise properties. Hartley tapped inductors were considered more difficult to integrate than Colpitts tapped capacitors, therefore fully-integrated Hartley oscillator is rarely implemented in CMOS technology. Nowdays fully-integrated tapped inductors are easy to design like other passive RF components, and Hartley oscillator may have good cyclostationary noise properties just like its Colpitts counterpart. In addition, the tapped inductor provides both dc and ac path, while the tapped capacitor provides only ac path, this will result in a simpler circuit in certain VCO topology [2]. Thus it is important to have a fully-integrated CMOS Hartley VCO with high performance for future application. 

\subsection{File for the old technology}
...
Main difference between class B and class C can be observed by investigating their drain currents. Because of this there is a difference in filtering the flicker noise coming from the bias providing tail current. 
...
\section{Questions about new double feedback}
...
\subsection{OTA2 - Start up and current bias control}

Referent voltage should be set and controlled around 400 mV. Problem with OTA2 loop is amplifying noise into the tail bias current. So the new problem arises as noise shaping in LOOP2 is needed. If a OTA of low uGBW (unity Gain BandWidth) is used than start up is too slow. \\

Removing this loop from the schematic shows improvement in phase noise that so the performance is similar as the original IMEC Scaldio design.
...
Expecting LO Range to not be centered comes from plot of Q and L comparisons shown in Fig. \ref{fig:Comparison_between_EMX_model_and_HFSS}.

\begin{figure}[!ht]
	\includegraphics[width=\linewidth]{Figures/Comparison_between_EMX_model_and_HFSS.png}
	\caption{Comparison between EMX and HFSS model}
	\label{fig:Comparison_between_EMX_model_and_HFSS}
\end{figure}
